...
describe('log', function () {
	// same category the logger derives: path of the module relative to the package parent folder
	const category = path.relative(path.join(__dirname, '../../'), __filename)
	...
	describe('error', function () {
		it('should use util.format for arg processing', function () {
			...
			const message = `${time.toISOString().white} ${'error'.red} ${category.cyan} obj {"a":1}`
	...
	describe('warn', function () {
		it('should use util.format for arg processing', function () {
			...
			const message = `${time.toISOString().white} ${'warn'.yellow} ${category.cyan} obj {"a":2}`
	...
	describe('debug', function () {
		it('should use util.format for arg processing', function () {
			...
			const message = `${time.toISOString().white} debug ${category.cyan} obj {"a":3}`
			consoleLog.should.be.calledWith(message)
		})
	})
})